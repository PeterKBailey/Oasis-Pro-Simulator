...
void Device::SetConnectionStatus(int status)
{
    ...
    if (state == State::TestingConnection && testConnectionTimer.remainingTime() <= 0 && prevStatus == ConnectionStatus::No)
    { // connect during test mode
    ...
    else if (state == State::Paused && connectionStatus != ConnectionStatus::No && disconnected)
    { // reconnect
        disconnected = false;
        this->resumeSession();
    }
